...
def evaluate_loss(data_loader, model, ntm_model, opt):
    ...
    with torch.no_grad():
        for batch_i, batch in enumerate(data_loader):
            ...
            # one2one setting
            beta = ntm_model.fcd1.weight.data.T
            decoder_dist, h_t, attention_dist, encoder_final_state, coverage, kld_loss, _, _ \
                = model(src, src_lens, trg, src_oov, max_num_oov, src_mask, topic_represent, beta)
...
def evaluate_beam_search(generator, one2many_data_loader, opt, delimiter_word='<sep>'):
    ...
    pred_output_file = open(pred_fn, "w",encoding='utf-8')
    # debug
    interval = 100
