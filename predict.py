...
import pykp.io
from Model import Seq2SeqModel, nTSNTM
...
def init_pretrained_model(opt):
    ...
    ntm_model = nTSNTM(1, 10, opt.bow_vocab_size, 256, opt.topic_num - opt.n_topic2 - 1, opt.n_topic2,
                       0, opt.batch_size, "sigmoid", opt.device).to(opt.device)
...
def process_opt(opt):
    ...
    opt.vocab = "processed_data/{}/".format("kp20k_s512_t10")
...
def main(opt):
    try:
        ...
        test_data_loader = load_test_data(opt, load_train=False)
