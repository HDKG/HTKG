...
def my_scatter_add(input_tensor: torch.Tensor, index_tensor: torch.Tensor, src: torch.Tensor) -> torch.Tensor:
    for i in range(index_tensor.shape[0]):
        for j in range(index_tensor.shape[1]):
            print(i,j,index_tensor[i][j])
            input_tensor[i][index_tensor[i][j]]+=src[i][j]
    return input_tensor
...
class RNNDecoder(nn.Module):
    ...
    def forward(self, y, h, memory_bank, src_mask, max_num_oovs, src_oov, coverage):
        ...
        if self.copy_attn:
            ...
            # final_dist=my_scatter_add(vocab_dist_,src_oov, attn_dist_)
