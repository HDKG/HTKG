...
def make_bow_dictionary(tokenized_src_trg_pairs, data_dir, bow_vocab):
    ...
    stopwords2 = read_stopwords("stopwords/stopwords.kp20k.txt")
...
def main(opt):
    ...
    if 'kp20k' in opt.data_dir:
        ...
        split = len(train_one2one) // 3
        torch.save(train_one2one[:split], open(opt.res_data_dir + '/train.one2one-0.pt', 'wb'))
        torch.save(train_one2one[split:2*split], open(opt.res_data_dir + '/train.one2one-1.pt', 'wb'))
        torch.save(train_one2one[2*split:], open(opt.res_data_dir + '/train.one2one-2.pt', 'wb'))
