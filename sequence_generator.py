...
class SequenceGenerator(object):
    ...
    def beam_search(self, src, src_lens, src_oov, src_mask, src_bow, oov_lists, word2idx, max_eos_per_output_seq=1):
        ...
        self.ntm_model.eval()
        # _, topic_represent, recon_batch, mu, logvar = self.ntm_model(src_bow_norm)
        # Encoding
        ...
        post_mu = []
        post_logvar = []
        for i in range(self.model.topic_num):
            post_mu.append(self.model.posterior_mu[i](decoder_state))
            post_logvar.append(self.model.posterior_logvar[i](decoder_state))
        decoder_state = self.model.reparameterize(sum(post_mu) / len(post_mu), sum(post_logvar) / len(post_mu))
        for t in range(1, self.max_sequence_length + 1):
            ...
            decoder_dist, decoder_state, context, attn_dist, _, coverage = \
                self.model.decoder(decoder_input, decoder_state, memory_bank, src_mask, max_num_oov,
                                   src_oov, coverage)
