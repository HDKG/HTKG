...
class Beam:
    ...
    def advance(self, word_logits, attn_dist):
        ...
        * `attn_dist`- attention at the last step [beam_size, src_len]
        ...
        prev_k = torch.floor_divide(best_scores_idx,
                                    vocab_size)  # convert it to the beam indices that the top k scores came from, LongTensor, size: [beam_size]
        ...
        for i in range(self.next_ys[-1].size(0)):  # For each generated token in the current step, check if it is EOS
            if self.next_ys[-1][i] == self._eos:
                ...
                if self.eos_counters[
                    i] == self.max_eos_per_output_seq:  # compute the score penalize by length and coverage amd append add it to finished
