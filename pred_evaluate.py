...
def main(opt):
    ...
    if opt.export_filtered_pred:
        pred_output_file = open(os.path.join(opt.filtered_pred_path, "predictions_filtered.txt"), "w",encoding='utf-8')
    ...
    t0 = time.time()
    for data_idx, (src_l, trg_l, pred_l) in enumerate(tqdm.tqdm(
            zip(open(src_file_path,encoding='utf-8'), open(trg_file_path,encoding='utf-8'), open(pred_file_path,encoding='utf-8')))):
        total_num_src += 1
